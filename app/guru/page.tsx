...
export default function GuruDashboard() {
  ...
  const [myRekap, setMyRekap] = useState([]);
  const [todayStatus, setTodayStatus] = useState({ masuk: null, pulang: null });
    ...
    try {
      ...
      if (resJson.success) {
        ...
        const grouped = rawData.reduce((acc: any, curr: any) => {
          ...
          if (!acc[dateKey]) {
            acc[dateKey] = { tanggal: dateKey, masuk: null, pulang: null };
          }
        ...
        const finalizedRekap = Object.values(grouped);
        setMyRekap(finalizedRekap as any);
    ...
    <div className="animate-in fade-in duration-500">
      ...
      <header className="mb-6 flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-6 rounded-[30px] shadow-sm border border-slate-100">
        <div className="flex items-center gap-4 w-full md:w-auto">
          <div className="w-14 h-14 bg-red-600 rounded-2xl flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-red-100">
            {profile?.nama_lengkap?.charAt(0) || "G"}
          </div>
          <div>
            <h1 className="text-xl font-black text-slate-800 uppercase tracking-tight">Halo, {profile?.nama_lengkap}</h1>
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{profile?.nip || 'NIP BELUM DIATUR'}</p>
          </div>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className={`p-6 rounded-[30px] border transition-all ${todayStatus.masuk ? 'bg-green-50 border-green-100' : 'bg-white border-slate-100'}`}>
          <div className="flex justify-between items-center">
            ...
            {todayStatus.masuk && <span className="text-[8px] bg-green-200 text-green-700 px-2 py-0.5 rounded font-black uppercase">Success</span>}
          </div>
          <h2 className={`text-3xl font-black mt-2 ${todayStatus.masuk ? 'text-green-600' : 'text-slate-200'}`}>
            {todayStatus.masuk ? new Date(todayStatus.masuk.waktu_absen).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'}) : '00:00'}
          </h2>
        </div>
        <div className={`p-6 rounded-[30px] border transition-all ${todayStatus.pulang ? 'bg-blue-50 border-blue-100' : 'bg-white border-slate-100'}`}>
          <div className="flex justify-between items-center">
            ...
            {todayStatus.pulang && <span className="text-[8px] bg-blue-200 text-blue-700 px-2 py-0.5 rounded font-black uppercase">Success</span>}
          </div>
          <h2 className={`text-3xl font-black mt-2 ${todayStatus.pulang ? 'text-blue-600' : 'text-slate-200'}`}>
            {todayStatus.pulang ? new Date(todayStatus.pulang.waktu_absen).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'}) : '00:00'}
          </h2>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-50">
              {myRekap.length > 0 ? myRekap.map((r: any, idx) => (
                <tr key={idx} className="hover:bg-slate-50/80 transition">
                  <td className="p-6 text-xs font-black text-slate-700 uppercase tracking-tighter">{r.tanggal}</td>
                  ...
                  <td className="p-6 text-center">
                    <span className="text-[8px] font-black px-3 py-1 bg-slate-100 text-slate-400 rounded-lg uppercase tracking-widest">
                      {r.masuk?.status || r.pulang?.status || 'Check'}
                    </span>
                  </td>
                </tr>
              )) : (
                <tr><td colSpan={4} className="p-20 text-center text-slate-300 font-black uppercase text-[10px] tracking-widest italic">Belum ada riwayat absensi bulan ini</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <form onSubmit={handleIzinSubmit} className="space-y-5">
            <div>
              ...
              <select 
                ...
                className="w-full mt-1 p-4 bg-slate-50 border-none rounded-2xl text-xs font-bold focus:ring-2 focus:ring-red-600 transition"
              >
            ...
            <div>
              ...
              <textarea 
                ...
                className="w-full mt-1 p-4 bg-slate-50 border-none rounded-2xl text-xs font-bold focus:ring-2 focus:ring-red-600 h-32 transition"
            ...
            <div>
              ...
              <input 
                ...
                className="w-full mt-1 p-3 text-xs text-slate-500 file:bg-red-50 file:text-red-600 file:border-none file:rounded-xl file:px-4 file:py-2 file:font-black file:mr-4 file:text-[9px] file:uppercase cursor-pointer"
              />
            </div>
