...
function DashboardContent() {
        ...
        const grouped = rawData.reduce((acc: any, curr: any) => {
          ...
          if (!acc[dateKey]) {
            acc[dateKey] = { 
              ...
              masuk: null, pulang: null, displayStatus: "HADIR", rawDate: new Date(curr.waktu_absen)
            };
          }
          const st = curr.status.toLowerCase();
          if (st.includes('masuk') || st.includes('terlambat')) {
            acc[dateKey].masuk = curr;
            acc[dateKey].displayStatus = curr.status.toUpperCase();
          } else if (st.includes('pulang')) { acc[dateKey].pulang = curr; }
          return acc;
        }, {});
  ...
  const handleIzinSubmit = async (e: React.FormEvent) => {
    ...
    // Validasi tambahan untuk Izin/Cuti
        ...
        <header className="mb-6 flex flex-col md:flex-row items-center justify-between gap-4 bg-white/90 backdrop-blur-md p-6 rounded-[30px] shadow-sm border border-slate-100">
          <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Dashboard Guru</p>
            </div>
          </div>
          <button onClick={() => router.push("/")} className="bg-slate-800 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest">🏠 Scanner</button>
        </header>
          ...
          <div className="bg-white/90 backdrop-blur-md rounded-[32px] shadow-xl border border-slate-100 overflow-hidden">
            <table className="w-full text-center">
              <thead className="bg-slate-800 text-white text-[9px] font-black uppercase tracking-widest">
                <tr><th className="p-6 text-left">Tanggal</th><th className="p-6">Masuk</th><th className="p-6">Pulang</th><th className="p-6">Status</th></tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {myRekap.map((r, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition">
                    <td className="p-6 text-left text-xs font-black text-slate-700">{r.tanggalFormat}</td>
                    <td className="p-6 text-xs font-black text-slate-600">{r.masuk ? new Date(r.masuk.waktu_absen).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'}) : '-'}</td>
                    <td className="p-6 text-xs font-black text-slate-600">{r.pulang ? new Date(r.pulang.waktu_absen).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'}) : '-'}</td>
                    <td className="p-6"><span className={`text-[8px] font-black px-3 py-1 rounded-lg uppercase ${r.displayStatus.includes('HADIR') ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}`}>{r.displayStatus}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="space-y-6">
            <div className="bg-white/90 p-8 rounded-[32px] shadow-xl border border-slate-100">
              ...
              <form onSubmit={handleIzinSubmit} className="space-y-4">
                ...
                {/* TAMPILKAN TANGGAL HANYA JIKA BUKAN SAKIT */}
                ...
                <div className="space-y-1">
                  ...
                  <textarea required value={formIzin.keterangan} onChange={e => setFormIzin({...formIzin, keterangan: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl text-xs font-bold h-24 outline-none ring-1 ring-slate-100 focus:ring-2 focus:ring-red-600" placeholder="Contoh: Izin acara keluarga di luar kota..."/>
                </div>

                <button type="submit" className="w-full bg-red-600 text-white py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-lg shadow-red-100 hover:bg-red-700 transition active:scale-95">Kirim Pengajuan</button>
              </form>
            </div>
                ...
                {myIzin.length > 0 ? myIzin.map((izin: any, idx: number) => (
                  <div key={idx} className="flex items-center justify-between p-5 bg-slate-50 rounded-[24px] border border-slate-100">
                    <div>
                      ...
                      {izin.tanggal_mulai && (
                        <p className="text-[8px] font-bold text-red-500 mt-1 uppercase tracking-tighter italic">Durasi: {izin.tanggal_mulai} s/d {izin.tanggal_selesai}</p>
                      )}
                    </div>
...
export default function GuruDashboard() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-black text-slate-300">SINKRONISASI...</div>}>
      <DashboardContent />
    </Suspense>
  );
}
